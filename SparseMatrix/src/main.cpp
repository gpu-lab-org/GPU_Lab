...
//#include "/home/muddasmd/GPU_Lab/Git_Project/ClSPARSE_Github/googletest/googletest/include/gtest/gtest.h"
#include "/home/muddasmd/GPU_Lab/Git_Project/WithClSparse/SparseMatrix/lib/ClSPARSE/clSPARSE-package/include/clSPARSE.h"
#include "/home/muddasmd/GPU_Lab/Git_Project/WithClSparse/SparseMatrix/lib/ClSPARSE/clSPARSE-package/include/clSPARSE-error.h"
/*
#include "/home/muddasmd/GPU_Lab/Git_Project/ClSPARSE_Github/clSPARSE/src/tests/resources/clsparse_environment.h"
#include "/home/muddasmd/GPU_Lab/Git_Project/ClSPARSE_Github/clSPARSE/src/tests/resources/csr_matrix_environment.h"
#include "/home/muddasmd/GPU_Lab/Git_Project/ClSPARSE_Github/clSPARSE/src/tests/resources/sparse_matrix_environment.h"
#include "/home/muddasmd/GPU_Lab/Git_Project/ClSPARSE_Github/clSPARSE/src/tests/resources/sparse_matrix_fill.hpp"
#include "/home/muddasmd/GPU_Lab/Git_Project/ClSPARSE_Github/clSPARSE/src/tests/resources/matrix_utils.h"
...
Eigen::SparseMatrix<float, Eigen::RowMajor,int> Dmatrix(cv::Mat& Src, cv::Mat & Dest, int rfactor)
{
	...
	size_t dim_srcvec = Src.rows * Src.cols;
    size_t dim_dstvec = Dest.rows * Dest.cols;
    ...
    _Dmatrix_ClSPARSE.values      = ::clCreateBuffer( context(), CL_MEM_READ_WRITE, _Dmatrix_ClSPARSE.num_nonzeros * sizeof( float ),               NULL, NULL );
    _Dmatrix_ClSPARSE.col_indices = ::clCreateBuffer( context(), CL_MEM_READ_WRITE, _Dmatrix_ClSPARSE.num_nonzeros * sizeof( clsparseIdx_t ),       NULL, NULL );
    _Dmatrix_ClSPARSE.row_pointer = ::clCreateBuffer( context(), CL_MEM_READ_WRITE, ( _Dmatrix_ClSPARSE.num_rows + 1 ) * sizeof( clsparseIdx_t ),   NULL, NULL );
    ...
    memset(h_GpuRp.data(), 0, (_Dmatrix_ClSPARSE.num_rows + 1) * sizeof(cl_uint));
    ::clEnqueueWriteBuffer(queue(), _Dmatrix_ClSPARSE.row_pointer, true, 0,  (_Dmatrix_ClSPARSE.num_rows + 1) * sizeof(clsparseIdx_t), h_GpuRp.data(), 0, NULL, &event_write3());
    ...
    ::clEnqueueWriteBuffer(queue(), NZ_values,  true, 0, count * sizeof(cl_float),  h_GpuV.data(), 0, NULL, &event_write1());
    ::clEnqueueWriteBuffer(queue(), NZ_Columns, true, 0, count * sizeof(cl_uint),   h_GpuC.data(), 0, NULL, &event_write2());
    ...
    //launch the kernel
    queue.enqueueNDRangeKernel(SuperAwesome_D_kernel, 0, cl::NDRange(Src.rows, Src.cols), cl::NDRange(wgSize, wgSize), NULL, &event_kernel);

    // Copy from device memory to ClSPARSE device memory
    cl_int copyres1 = clEnqueueCopyBuffer(queue(), NZ_values, _Dmatrix_ClSPARSE.values, 0, 0, _Dmatrix_ClSPARSE.num_nonzeros * sizeof(float), 0, NULL, &event_copy1());
    cl_int copyres2 = clEnqueueCopyBuffer(queue(), NZ_Columns, _Dmatrix_ClSPARSE.col_indices, 0, 0, _Dmatrix_ClSPARSE.num_nonzeros * sizeof(clsparseIdx_t), 0, NULL, &event_copy2());
    
    if(copyres1 == CL_SUCCESS)
        std::cout << "copyres1 value " << copyres1 << "\n";
	...
	Core::TimeSpan time2 = OpenCL::getElapsedTime(event_copy1);
	Core::TimeSpan time3 = OpenCL::getElapsedTime(event_copy2);
	//Core::TimeSpan time4 = OpenCL::getElapsedTime(event_copy3);
	...
	Core::TimeSpan timeGPU = time1+time2+time3+time5+time6+time7;

    std::cout <<"Kernel Time "<< time1 << std::endl;
    std::cout <<"Copy1 Time "<< time2 << std::endl;
    std::cout <<"Copy2 Time "<< time3 << std::endl;
    //std::cout <<"Copy3 Time "<< time4 << std::endl;
    ...
    std::vector<cl_float> IVal(dim_dstvec);
    std::vector<cl_uint> ICol(dim_dstvec);
    std::vector<cl_uint> IRowp(dim_dstvec+1);
    ...
    // Define a Identity SPARSE matrix
    clsparseCsrMatrix _Imatrix_ClSPARSE;
    clsparseInitCsrMatrix(&_Imatrix_ClSPARSE);

    // Declaration of CSR matrix parameters
    clsparseIdx_t innz = dim_dstvec;
    clsparseIdx_t irow = dim_dstvec;
    clsparseIdx_t icol = dim_dstvec;

    // Initialised _Dmatrix parameters
    _Imatrix_ClSPARSE.num_nonzeros  = innz; 
    _Imatrix_ClSPARSE.num_rows      = irow; 
    _Imatrix_ClSPARSE.num_cols      = icol;

    // Allocate memory for _Dmatrix CSR matrix
    _Imatrix_ClSPARSE.values      = ::clCreateBuffer( context(), CL_MEM_READ_WRITE, _Imatrix_ClSPARSE.num_nonzeros * sizeof( float ),               NULL, NULL );
    _Imatrix_ClSPARSE.col_indices = ::clCreateBuffer( context(), CL_MEM_READ_WRITE, _Imatrix_ClSPARSE.num_nonzeros * sizeof( clsparseIdx_t ),       NULL, NULL );
    _Imatrix_ClSPARSE.row_pointer = ::clCreateBuffer( context(), CL_MEM_READ_WRITE, ( _Imatrix_ClSPARSE.num_rows + 1 ) * sizeof( clsparseIdx_t ),   NULL, NULL );

    // Writing row pointer from host into device
    ::clEnqueueWriteBuffer(queue(), _Imatrix_ClSPARSE.values, true, 0,  _Imatrix_ClSPARSE.num_nonzeros * sizeof(float), IVal.data(), 0, NULL, NULL);

    // Writing row pointer from host into device
    ::clEnqueueWriteBuffer(queue(), _Imatrix_ClSPARSE.col_indices, true, 0,  _Imatrix_ClSPARSE.num_nonzeros * sizeof(clsparseIdx_t), ICol.data(), 0, NULL, NULL);

    // Writing row pointer from host into device
    ::clEnqueueWriteBuffer(queue(), _Imatrix_ClSPARSE.row_pointer, true, 0,  ( _Imatrix_ClSPARSE.num_rows + 1 ) * sizeof(clsparseIdx_t), IRowp.data(), 0, NULL, NULL);
    ...
    {
        //std::cout << "(" << h_GpuC[i] <<")" << "-->" << h_GpuV[i] << "--> ; ( " << test_values2[i]  <<")" << " ---> " << test_values1[i] << "\n";
        //std::cout << "( " << test_valuesc[i] << " , " << test_valuesb[i] << ")" << "-->" << test_valuesa[i] << "\t";
    }

    // Define a Result SPARSE matrix
    clsparseCsrMatrix _Rmatrix_ClSPARSE;
    clsparseInitCsrMatrix(&_Rmatrix_ClSPARSE);

    // Declaration of CSR matrix parameters
    clsparseIdx_t rnnz = Src.rows * Src.cols * rfactor * rfactor;
    clsparseIdx_t rrow = dim_srcvec;
    clsparseIdx_t rcol = dim_dstvec;

    // Initialised _Dmatrix parameters
    _Rmatrix_ClSPARSE.num_nonzeros  = rnnz; 
    _Rmatrix_ClSPARSE.num_rows      = rrow; 
    _Rmatrix_ClSPARSE.num_cols      = rcol;

    // Allocate memory for _Dmatrix CSR matrix
    _Rmatrix_ClSPARSE.values      = ::clCreateBuffer( context(), CL_MEM_READ_WRITE, _Rmatrix_ClSPARSE.num_nonzeros * sizeof( float ),               NULL, NULL );
    _Rmatrix_ClSPARSE.col_indices = ::clCreateBuffer( context(), CL_MEM_READ_WRITE, _Rmatrix_ClSPARSE.num_nonzeros * sizeof( clsparseIdx_t ),       NULL, NULL );
    _Rmatrix_ClSPARSE.row_pointer = ::clCreateBuffer( context(), CL_MEM_READ_WRITE, ( _Rmatrix_ClSPARSE.num_rows + 1 ) * sizeof( clsparseIdx_t ),   NULL, NULL );

    // Finally MULTIPLICATION in SPGEMM!!!!!!!!!!!!!!

    // Inits and includes
    //using CLSE = ClSparseEnvironment;

    clsparseStatus status_mul = clsparseScsrSpGemm (&_Dmatrix_ClSPARSE, &_Imatrix_ClSPARSE , &_Rmatrix_ClSPARSE, NULL);

/*
    EXPECT_EQ(clsparseSuccess, status_mul);
...
*/
    ...
    std::cout << "status_mul is - " << status_mul << "\n";
    if(status_mul == clsparseSuccess)
            std::cout << "status_mul is - " << status_mul << "\n";
    ...
    k = 0;
    for (int i = 0; i < _Rmatrix_ClSPARSE.num_nonzeros; i++)
    {
        //std::cout << "(" << h_GpuC[i] <<")" << "-->" << h_GpuV[i] << "--> ; ( " << test_values2[i]  <<")" << " ---> " << test_values1[i] << "\n";
        std::cout << "( " << test_valuesm[i] << ")" << "-->" << test_valuesl[i] << "\t";
    }


    // Release I matrix ClSPARSE memory
    clReleaseMemObject ( _Rmatrix_ClSPARSE.values );
    clReleaseMemObject ( _Rmatrix_ClSPARSE.col_indices );
    clReleaseMemObject ( _Rmatrix_ClSPARSE.row_pointer );

    // Release I matrix ClSPARSE memory
    clReleaseMemObject ( _Imatrix_ClSPARSE.values );
    clReleaseMemObject ( _Imatrix_ClSPARSE.col_indices );
    clReleaseMemObject ( _Imatrix_ClSPARSE.row_pointer );


    // Release ClSPARSE memory
    clReleaseMemObject ( _Dmatrix_ClSPARSE.values );
    clReleaseMemObject ( _Dmatrix_ClSPARSE.col_indices );
    clReleaseMemObject ( _Dmatrix_ClSPARSE.row_pointer );
