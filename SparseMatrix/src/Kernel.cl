...
__kernel void SuperAwesome_H_Row_Pointer (int psfW, uint Dest_rows, uint Dest_cols, __global int* NZ_Row_Pointer) {
	...
	uint yact = (i % Dest_cols);                             // Y value of Dest matrix
    ...
    else 
    {
        ...
        // Handle corner elements
        else if(((x - (psfWidth / 2)) < 0) && ((y + (psfWidth / 2)) > (Dest_rows - 1)))
        {
            NZ_Row_Pointer[i] = ( (psfWidth + (x - (psfWidth / 2))) * (psfWidth - (y + (psfWidth / 2) - (Dest_rows - 1))) );
        }

        // Handle corner elements
        else if(((x + (psfWidth / 2)) > (Dest_cols - 1)) && ((y - (psfWidth / 2)) < 0))
        {
            NZ_Row_Pointer[i] = ( (psfWidth - (x + (psfWidth / 2) - (Dest_cols - 1))) * (psfWidth + (y - (psfWidth / 2))) );
        }

        // Handle corner elements
        else if(((x + (psfWidth / 2)) > (Dest_cols - 1)) && ((y + (psfWidth / 2)) > (Dest_rows - 1)))
        {
            NZ_Row_Pointer[i] = ( (psfWidth - (x + (psfWidth / 2) - (Dest_cols - 1))) * (psfWidth - (y + (psfWidth / 2) - (Dest_rows - 1))) );
        }

        // Handle edge elements - X < 0, Y in range
        else if( ((x - (psfWidth / 2)) < 0) && (((y - (psfWidth / 2)) >= 0) && ((y + (psfWidth / 2)) <= (Dest_rows - 1))) )
        {
            NZ_Row_Pointer[i] = ( (psfWidth + (x - (psfWidth / 2))) * psfWidth );
        }

        // Handle edge elements - X > max, Y in range
        else if( ((x + (psfWidth / 2)) > (Dest_cols - 1)) && (((y - (psfWidth / 2)) >= 0) && ((y + (psfWidth / 2)) <= (Dest_rows - 1))) )
        {
            NZ_Row_Pointer[i] = ( (psfWidth - (x + (psfWidth / 2) - (Dest_cols - 1))) * psfWidth );
        }

        // Handle edge elements - X in range, Y < 0
        else if( ((x - (psfWidth / 2)) >= 0) && ((x + (psfWidth / 2)) <= (Dest_cols - 1)) && ((y - (psfWidth / 2)) < 0) )
        {
            NZ_Row_Pointer[i] = ( psfWidth * (psfWidth + (y - (psfWidth / 2))) );
        }

        // Handle edge elements - X in range, Y > max
        else if( ((x - (psfWidth / 2)) >= 0) && ((x + (psfWidth / 2)) <= (Dest_cols - 1)) && ((y + (psfWidth / 2)) > (Dest_rows - 1)))
        {
            NZ_Row_Pointer[i] = ( psfWidth * ((psfWidth - (y + (psfWidth / 2) - (Dest_rows - 1)))) );
        }

        // Rest of the elements - X in range, Y in range
        else
        {
...
*/
__kernel void SuperAwesome_M_Matrix (float deltaX, float deltaY, __global float* NZ_values, __global int* NZ_Columns, __global int* NZ_Row_Pointer) {
    ...
    if(i < (countX-floor(deltaY)) && j< (countY-floor(deltaX)) && (i+floor(deltaY) >= 0) && (j+floor(deltaX) >= 0))
	{
        ...
        NZ_Row_Pointer[i] = R_Pointer_cnt;
	}
}
